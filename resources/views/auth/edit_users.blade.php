...
@section("content")
    <div class="container-fluid min-vh-100 d-flex flex-column justify-content-center">
        ...
        <br><br>
        {{-- <button><p class="text-end"> + Add Edits</p> --}}
        @if ($errors->any())
            <div>
                {{-- <li>{{ $error }}</li> --}}
        ...
        <form class="row g-3 w-50 mx-auto" method="POST" action="{{ route("editbook.post", $user->id) }}">
            @csrf
            ...
            <div class="mb-3 mt-3 row">
                ...
                <div class="col-sm-8">
                    <input type="text" class="form-control" id="author" name="author" required value="{{ $user->author }}">
                </div>
            </div>
            <div class="mb-3 mt-3 row">
                <label for="publication-company" class="col-sm-4 col-form-label">Publication Co.</label>
                <div class="col-sm-8">
                    <input type="text" class="form-control" id="publication_company" name="publication_company" required value="{{ $user->publication_company }}">
                </div>
            </div>
            <div class="mb-3 mt-3 row">
                <label for="publication-date" class="col-sm-4 col-form-label">Publication Date</label>
                <div class="col-sm-8">
                    <input type="date" class="form-control" id="publication_date" name="publication_date" max="9999-12-31" required value="{{ $user->publication_date }}">
                </div>
            </div>
